...
from bokeh.layouts import gridplot, widgetbox, layout, column
...
def run_dashboard(tr, graph_, edges_, nodes, reportdate, log=log, datesnum = 10, 
                  ...
                  nodes_stroke_color = TTQcolor['sixtyGrey'], 
                  edges_colors = [TTQcolor['eightyGrey'], TTQcolor['warningRed']], #to be removed
                  patterns_latecol = TTQcolor["richOrange"], 
                  patterns_maincol = TTQcolor["sixtyGrey"], #used for boundary only atm
                  patterns_earlycol = TTQcolor["algae"], 
                  warningcol = TTQcolor['warningRed'],
                  replinecol = TTQcolor["yellowOrange"],
                  score1_ts = TTQcolor['ocean'],
                  score2_ts = TTQcolor['richOrange'],
                  score3_ts = TTQcolor['warningRed'],
                  alphanode_choices = [0.9, 0.15],
                  widthnode_choices = [0.25, 0.1],
                  widthedge_choices = [0.99, 0.2],
                  alphaedge_choices = [0.99, 0.15],
                  nodes_size_range = (3.5,15),
                  edges_selected_col = TTQcolor['whiteGrey'], #to be removed
                  ...
                  TS_TOOLTIPS = [('customer_name', '@customers_name'),
                                 ('debtor_name', '@debtor_name'),
                                 ('invoice id', '@uid'),
                             ...
                             'is_open': tr['is_open'],
                                ...
                                'paid_amount':tr['paid_amount_forVIZ'],
                                    ...
                                    'has_impairment1':tr['has_impairment1'],
                                     ...
                                     'delay':tr['payment_date_mismatch']
                                    })

    log.info("Setting up repayment patterns figure attributes...")
    #axis ranges
    y_range = (np.min(source.data['left1']), max(reportdate+15, np.nanmax(source.data['right2'])))
    x_range = (-.5, max(source.data['y'])+.5)
    
    #main fig and hovertool
    fig = figure(x_range=x_range, y_range=y_range, plot_width=1200, plot_height=400,
                tools = _tools_to_show_rp, background_fill_color = background_color, title='REPAYMENT PATTERNS FOR SELECTED SUB-NETWORK')
    ...
    linedict = {} #line dictionary to remove them after figure creation avoiding overlap
    bardict = {} #same for bar graphs
    ...
    # Time Series
    # --------------------------------------------------------------------------------
    log.info("Setting up timeseries figure attributes...")
    ts = figure(width=1200, height=400, x_range=x_range, y_range=(-0.1,1.3), title='TTQ MODEL REACTION FOR SELECTED SUB-NETWORK', 
                tools=_tools_to_show_rp)#, x_axis_type='datetime')
    #legend settings
    
    #ts.add_layout(color_bar, 'right')
    ts.xaxis.axis_label = 'Time - Invoice date'
    ts.yaxis.axis_label = 'TTQ Score'
    circledict={} #circles dictionary in order to remove them after each figure creation avoiding overlap
    timelinedict={} #same for time series lines
    ...
    def show_patterns(df_cust, to_highlight):
        """
        This function includes all the graphic updates at each selection for the repayment patterns and timeseries.
                             ...
                             'is_open': df_cust['is_open'],
                                ...
                                'paid_amount':df_cust['paid_amount_forVIZ'],
                                    ...
                                    'has_impairment1':df_cust['has_impairment1'],
                                     ...
                                     'delay':df_cust['payment_date_mismatch']
                                    }
        #axis ranges
        y_range_new = (np.min(source.data['left1']), max(reportdate+15, np.nanmax(source.data['right2'])))
        x_range_new = (-.5, max(source.data['y'])+.5)
        ...
        #bars update
        if len(bardict.keys())>0:
            for bb in bardict.keys():
                fig.renderers.remove(bardict[bb])

        #mainbar
        bar_1 = fig.vbar(x = 'y', bottom = 'left1', top='right1', width = .8, line_color = 'imp_colors_2', line_width='widthwarning', #line_alpha='alphawarning', #width wiwth rules
                    fill_color='imp_colors_2', alpha=.8,  source=source, nonselection_fill_color=TTQcolor['eightyGrey'],
                    nonselection_line_alpha=0)
        bardict['bar_1'] = bar_1

        #late repayment bar
        bar_2 = fig.vbar(x = 'y', bottom = 'left2', top='right2', width = .8, 
        ...
        #early repayment bar
        bar_3 = fig.vbar(x = 'y', bottom = 'left3', top='right3', width = .8, line_color='imp_colors_3', line_width=0.0,  line_alpha=0.,
        ...
        #report date line
        fig.line(y=[reportdate, reportdate], x=[min(y)-1, max(y)+1], 
                    line_color=replinecol, line_dash='dashed')

        #hover tool
        daystmp = np.arange(y_range_new[0], y_range_new[1]).tolist() #list of dates into the range
        ...
        coords = {}
        coords['ys'] = [[d,d] for d in daystmp[::no_of_dates] if (datetime.fromordinal(int(d))+days_margin < datetime.fromordinal(int(reportdate)) or 
                                                                  datetime.fromordinal(int(d))-days_margin> datetime.fromordinal(int(reportdate)))]
        coords['xs'] = [[min(y)-1, max(y)+1] for d in daystmp[::no_of_dates] if (datetime.fromordinal(int(d))+days_margin < datetime.fromordinal(int(reportdate)) or 
                                                                                 datetime.fromordinal(int(d))-days_margin> datetime.fromordinal(int(reportdate)))]
        ...
        fig.yaxis.ticker = daynums
        fig.yaxis.major_label_overrides = dict(zip(daynums, [datetime.fromordinal(int(s)).strftime("%Y-%m-%d") for s in daynums]))
        fig.ygrid.grid_line_color=None
        fig.xgrid.grid_line_color=None
        fig.yaxis.axis_label = 'Time'

        fig.add_tools(HoverTool(tooltips =RP_TOOLTIPS))

        #------------------------
        # changes on time series
        #------------------------
        #timelines update
        if len(timelinedict.keys())>0:
            for ll in timelinedict.keys():
                ts.renderers.remove(timelinedict[ll])

        ts.x_range.start = 0
        ts.x_range.end = len(df_cust.invoice_date)

        source.data['x_pos'] = [p for p in range(len(df_cust.invoice_date))] #np.linspace(y_range_new[0], y_range_new[1], len(df_cust)) #creating x coords for time series

        source.data['x_pos_shift'] = [g+0.5 for g in range(len(df_cust.invoice_date))]

        ts_1=ts.line(x ='x_pos_shift' , y='score_0', line_width=2.5, alpha=0.85, source=source, color=score1_ts, muted_alpha = 0.15, muted_color =score1_ts )
        timelinedict['ts_1'] = ts_1

        ts_2=ts.line(x ='x_pos_shift' , y='score_1', line_width=2.5, alpha=0.85, source=source, color=score2_ts, muted_alpha = 0.15, muted_color =score2_ts )
        timelinedict['ts_2'] = ts_2

        ts_3=ts_store = ts.line(x ='x_pos_shift' , y='score_2', line_width=2.5, alpha=0.85, source=source, color=score3_ts, muted_alpha = 0.15, muted_color =score3_ts )
        timelinedict['ts_3'] = ts_3
        
        if len(ts.legend)>0:
            circle_placeholder = ts.circle(x=-10, y=0, size=8, fill_color=warningcol, source=source)
            ts.legend[0].items= [('impairment1_score',[ts_1]), ('pastdue90_score',[ts_2]), ('pastdue180_score',[ts_3]), ('credit event', [circle_placeholder])]
        else:
            circle_placeholder = ts.circle(x=-10, y=0, size=8, fill_color=warningcol, source=source)
            tslegend= Legend(items=[('impairment1_score',[ts_1]), ('pastdue90_score',[ts_2]), ('pastdue180_score',[ts_3]), ('credit event', [circle_placeholder])])
            ts.add_layout(tslegend, 'right')

        if to_highlight!='None':
            #circles update
            if len(circledict.keys())>0:
                for b in circledict.keys():
                    try:
                        ts.renderers.remove(circledict[b])
                    except ValueError:
                        log.info('No glyph to remove - the script will keep going..')
                        pass

            source.data['dots_color'] = [warningcol if i else 'white'  for i in source.data[to_highlight]]

            if to_highlight=='has_impairment1':
                circles = Circle(x = 'x_pos_shift', y='score_0', line_alpha='has_impairment1', fill_alpha='has_impairment1',
                                 size=8, fill_color='dots_color')
                circledict['circles']=ts.add_glyph(source, circles)

            elif to_highlight=='is_pastdue90':
                circles = Circle(x = 'x_pos_shift', y='score_1', line_alpha='is_pastdue90', fill_alpha='is_pastdue90',
                                 size=8, fill_color='dots_color')
                circledict['circles']=ts.add_glyph(source, circles)

            elif to_highlight=='is_pastdue180':
                circles = Circle(x = 'x_pos_shift', y='score_2', line_alpha='is_pastdue180', fill_alpha='is_pastdue180',
                                 size=8, fill_color='dots_color')
                circledict['circles']=ts.add_glyph(source, circles)
            ...
            #circles update
            if len(circledict.keys())>0:
                for bb in circledict.keys():
                    try:
                        ts.renderers.remove(circledict[bb])
                    except ValueError:
                        log.info('No glyph to remove - the script will keep going..')
                        pass
        
        #xaxis in datetime
        daystmp_x = list(df_cust.invoice_date.apply(lambda x: x.toordinal())) #np.arange(y_range_new[0], y_range_new[1]).tolist() #list of dates into the range

        idxssel = range(len(source.data['x_pos_shift']))

        if len(idxssel) >15:
            idxssel = [kk for kk in idxssel if kk%(int(len(idxssel)//15))==0]

        ts.axis.ticker = [source.data['x_pos_shift'][kk] for kk in idxssel]
        ts.xaxis.major_label_overrides = dict(zip(source.data['x_pos_shift'], [datetime.fromordinal(int(s)).strftime("%Y-%m-%d") for s in daystmp_x]))
        ts.xaxis.major_label_orientation = -math.pi/4

        #hovertool
        ts.add_tools(HoverTool(tooltips =TS_TOOLTIPS))

        ts.legend.location = "top_left"
        ts.legend.click_policy="mute"
    ...
    l = gridplot([[widget_highlight_select, widget_degree_select, empty, widget_cd_select_button, None,spinner], 
               [plot, patterns_series]], sizing_mode='fixed') #, merge_tools=False)
