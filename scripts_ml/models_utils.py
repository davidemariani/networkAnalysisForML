...
def models_loop(models, datafolder, prefixes, postfixes, val_timeseq_encoding, val_postfixes, trainfile='_traindata', testfile='_testdata', scoring = {'AUC':'roc_auc'},
                save_model=False, models_path='../data/models/', mlf_tracking=False, experiment_name='experiment',
                save_results_for_viz=False, viz_output_path='../data/viz_data/'):
    """
    This function's main purpose is running a full experiment from validation to testing for evaluating performances preventing time leak.
    It requires as inputs:
    - models: a list of models
    - datafolder: the path of the folder containing train and test files generated by the preprocessing pipeline
    - prefixes: the prefix for each of them (list)
    - postfixes: the postfixes for each of them (list)
    - val_timeseq_encoding: the portion of the validation file's name indicating the windows sizes used
